...
int copy_from_user(void *kaddr, const void *uaddr, size_t nbytes)
{
        ...
        int res = vmmap_read(curproc->p_vmmap, uaddr, kaddr, nbytes);
        return res;
}
...
int copy_to_user(void *uaddr, const void *kaddr, size_t nbytes)
{
        ...
        int res = vmmap_write(curproc->p_vmmap, uaddr, kaddr, nbytes);
        return res;
}
...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
   ...
   /* uint32_t vfn = ADDR_TO_PN(vaddr);
    ...
    if ( (vma != NULL) && ((vma->vma_prot & perm) == perm) ) {
        dbg(DBG_PRINT, "(GRADING3F addr_perm.1)\n");
        return 1;
    }
    dbg(DBG_PRINT, "(GRADING3F addr_perm.2)\n");*/
   // return 0;
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
    ...
    char* add = (char*)avaddr;
    while(add < (char*)avaddr+len){
            vmarea_t *vma = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(add));
            if(vma == NULL) {
