...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
	...
	/* Yue code begins */
        ...
        memset(kthread,0,sizeof(kthread_t));
        char *kstack = alloc_stack();
        size_t kstacksz = DEFAULT_STACK_SIZE;
        pagedir_t *pdptr = p->p_pagedir;
        context_t ctx;
        context_setup(&ctx, func, arg1, arg2,kstack, kstacksz, pdptr);
        kthread->kt_ctx = ctx;
        kthread->kt_kstack = kstack;
        kthread->kt_retval = 0;
        kthread->kt_errno = 0;
        kthread->kt_proc = p;
...
kthread_cancel(kthread_t *kthr, void *retval)
{
        ...
        KASSERT(NULL != kthr); /* please use TWO consecutive "conforming dbg() calls" for this */
        /*dbg(DBG_PRINT,"(GRADING1A 3.b) thread not null\n");*/
...
kthread_clone(kthread_t *thr)
{
        ...
        /* the new thread needs its own context and stack*/
        char *kstack = alloc_stack();
        /*size_t kstacksz = DEFAULT_STACK_SIZE;
    
        pagedir_t *pdptr = thr->kt_proc->p_pagedir;
        context_t ctx;
        context_setup(&ctx, NULL, 0, NULL,kstack, kstacksz, pdptr);
        newthr->kt_ctx = ctx; */
       ...
       /* if (newthr->kt_wchan){
            ...
            dbg(DBG_PRINT,"(GRADING3F kthread_clone().1)\n");
        }*/
    
    
        //list_insert_tail(&thr->kt_proc->p_threads,&newthr->kt_plink);
