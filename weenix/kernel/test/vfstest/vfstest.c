...
vfstest_mkdir(void)
{
        ...
        syscall_fail(mkdir(LONGNAME, 0777), ENAMETOOLONG); //Wellen 34 vs 1024
...
#define CHDIR_TEST_DIR "chdir"
        ...
        test_assert(sdest.st_ino == sdir.st_ino, NULL); // Wellen
        test_assert(ssrc.st_ino == sparent.st_ino, NULL);
        test_assert(ssrc.st_ino != sdest.st_ino, NULL); //Wellen
...
#define HUGE_FD 9999
        ...
        syscall_fail(read(BAD_FD, buf, FD_BUFSIZE), EBADF); //Wellen
