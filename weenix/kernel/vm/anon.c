         ...
         list_iterate_begin(&(o->mmo_respages), pf, pframe_t,pf_olink)
                        {
                               /* if(pframe_is_busy(pf))
                                {
                                       ...
                                       dbg(DBG_PRINT,"(GRADING3D anon_put().1)\n");
                                }*/
                               ...
                               /* if(pframe_is_dirty(pf))
                                {
                                        pframe_clean(pf);
                                        dbg(DBG_PRINT,"(GRADING3D anon_put().3)\n");
                                }*/
                                pframe_free(pf);
                                dbg(DBG_PRINT,"(GRADING3A)\n");
                        }list_iterate_end();
...
anon_lookuppage(mmobj_t *o, uint32_t pagenum, int forwrite, pframe_t **pf)
{
       ...
       pframe_t *pft=NULL;
     int ret=pframe_get(o,pagenum,&pft);
                    /* if(pframe_is_busy(pft))
                {
                        sched_sleep_on(&pft->pf_waitq);
                        dbg(DBG_PRINT,"(GRADING3D anon_lookuppage().1)\n");
                }*/
                *pf=pft;
                dbg(DBG_PRINT,"(GRADING3A)\n");
                return 0;
        
        /*dbg(DBG_PRINT,"(GRADING3D anon_lookuppage().3)\n");
        return -1; */
}
